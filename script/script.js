...
var candies_arr = [];
var candy_imgs = ['url(../imgs/1.png)','url(../imgs/2.png)','url(../imgs/3.png)','url(../imgs/4.png)', 'url(../imgs/5.png)','url(../imgs/6.png)'];
var candyDraggedImg;
    ...
    for (i = 0; i < 56; i ++) {
      //if the square below has no image
        if(candies_arr[i + num_of_icons_in_row].style.backgroundImage === '') {
          ...
          candies_arr[i + num_of_icons_in_row].style.backgroundImage = candies_arr[i].style.backgroundImage;
...
function checkFourInRow() {
  ...
  for (i = 0; i < 61; i ++) {
    ...
    if(candies_arr[rowOfFour[0]].style.backgroundImage === candy_img_check && 
...
function checkFourInColumn() {
  for (i = 0; i < 40; i ++) {
    var columnOfFour = [i, i+num_of_icons_in_row, i+num_of_icons_in_row*2, i+num_of_icons_in_row*3]
    var candy_img_check = candies_arr[i].style.backgroundImage;
    var isBlank = candies_arr[i].style.backgroundImage === '';

    if(candies_arr[columnOfFour[0]].style.backgroundImage === candy_img_check && 
...
for (i = 0; i < 62; i ++) {
  ...
  if(candies_arr[rowOfThree[0]].style.backgroundImage === candy_img_check && 
...
for (i = 0; i < 48; i ++) {
  var columnOfThree = [i, i+num_of_icons_in_row, i+num_of_icons_in_row*2];
  var candy_img_check = candies_arr[i].style.backgroundImage;
  var isBlank = candies_arr[i].style.backgroundImage === '';

  if(candies_arr[columnOfThree[0]].style.backgroundImage === candy_img_check && 
...
var timer3 = setInterval(function(){
  ...
  if(score>=100 && counter<100){
    ...
    isfirstwin=false;
  } else if(counter>100 && result.innerHTML =="You Win!" && startscreen.style.opacity == 1){
    result.innerHTML ="You Win!";
  }
